...
// Extract video-specific data from the TikTok page HTML
function extractVideoData(html) {
  ...
  for (const pattern of patterns) {
    ...
    try {
      ...
      } else if (data.id && item.author?.uniqueId) {
        // Fallback construction using author uniqueId
        data.canonicalUrl = `https://www.tiktok.com/@${item.author.uniqueId}/video/${data.id}`;
      }

      // Quality extraction from bitrateInfo
      ...
      // Fallback to playAddr if no bitrateInfo entries
      ...
      // Set primary download URL as first available
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.statusCode = 500;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ error: 'Failed to fetch video data', details: error.message }));
  }
};
