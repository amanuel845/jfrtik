...
module.exports = async function handler(req, res) {
  ...
  // Build the URL. If uniqueId is provided, use the canonical form;
  // otherwise fall back to the embed endpoint that only needs the id.
  let targetUrl;
  if (uniqueId) {
    targetUrl = `https://www.tiktok.com/@${encodeURIComponent(uniqueId)}/video/${videoId}`;
  } else {
    targetUrl = `https://www.tiktok.com/embed/v2/${videoId}`;
  }
